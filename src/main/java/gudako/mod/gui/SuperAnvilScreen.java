...
@OnlyIn(Dist.CLIENT)
public class SuperAnvilScreen extends AbstractRepairScreen<SuperAnvilContainer> {
    ...
    public SuperAnvilScreen(SuperAnvilContainer p_i51103_1_, PlayerInventory p_i51103_2_, ITextComponent p_i51103_3_) {
    ...
    protected void renderLabels(MatrixStack p_230451_1_, int p_230451_2_, int p_230451_3_) {
        ...
        int i = ((SuperAnvilContainer)this.menu).getCost();
        if (i > 0) {
            int j = 8453920;
            Object itextcomponent;
            if (i >= 2147483647 && !this.minecraft.player.abilities.instabuild) {
                itextcomponent = TOO_EXPENSIVE_TEXT;
                j = 16736352;
            } else if (!((SuperAnvilContainer)this.menu).getSlot(2).hasItem()) {
                itextcomponent = null;
            } else {
                itextcomponent = new TranslationTextComponent("container.repair.cost", new Object[]{i});
                if (!((SuperAnvilContainer)this.menu).getSlot(2).mayPickup(this.inventory.player)) {
                    j = 16736352;
                }
            }

            if (itextcomponent != null) {
                int k = this.imageWidth - 8 - this.font.width((ITextProperties)itextcomponent) - 2;
                @SuppressWarnings("unused")
				boolean l = true;
                fill(p_230451_1_, k - 2, 67, this.imageWidth - 8, 79, 1325400064);
                this.font.drawShadow(p_230451_1_, (ITextComponent)itextcomponent, (float)k, 69.0F, j);
            }
        }
